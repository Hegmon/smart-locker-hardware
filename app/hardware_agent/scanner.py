...
# DEBUG LOGIC
#=====================================================================================
def _duduplicate(self,networks:list[WifiNetwork])->list[WifiNetwork]:
    duduped:dict[str,WifiNetwork]={}

    for n in networks:
        if not  n.ssid:
            continue
        existing=duduped.get(n.ssid)

        if existing is None or n.rssi > existing.rssi:
            duduped[n.ssid]
        
        return sorted(duduped.values(),key=lambda x:(-x.rssi,x.ssid))
...
def _scan_with_nmcli(self)->list[WifiNetwork]:
    subprocess.run(
        ["nmcli","dev","wifi","rescan","ifame",self.interface])
...
def _parse_iw(self,ouput:str)->Iterable[WifiNetwork]:
    ssid=""
    signal=None
    secured=False
    for line in ouput.splitlines():
        line=line.strip()
